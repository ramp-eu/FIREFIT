...
>***Note that at the current state only MongoDB is considered, but the component was designed to scale to other types of database services in the furute.***
...
To ensure proper functionality, ICN depends on the following services:
...
ICN imposes that a [Image Reference data model](../GCN/data_models/image_reference.json) needs to be used to ensure its functionality, this requirements is fulfilled if the [Generic Camera Node (GCN)](../GCN) is used. As mentioned in [FIREFIT Rose-AP repository documentation](../README.md), the ICN and GCN were designed to work together, presenting an ideal synergy. Despite said synergy, they not have direct dependencies and allow usage as standalone services.

# 4. Configuration
There are three files that need to be configured in order to use ICN:

- [`configuration.json`](#41-configuration-json) - this Json file holds all the necessary parameters to configure ICN and how it connects to other services.
- [`classifier.py`](#42-classifier-py) - this Python file is responsible for holding all the necessary code to perform the image classification tasks which may include data augmentation or preprocessing.
- [`Dockerfile`](#43-dockerfile) - this file is responsible for holding all the necessary elements to create a Docker image containing ICN.

Keep in mind the [current limitations of ICN](#44-current-limitations) through the configuration process.
...
Reading the [Generic Camera Node (GCN) documentation](../GCN/README.md) is, for the above reasons, highly recommended and it will be assumed for the current example that such knowledge is acquired.
...
Having a deployment that ensures the minimal service requirements for GCN and ICN, the next step is to configure the necessary files before building the GCN and ICN images.

Starting with GCN, the same approach taken for the component [usage example](../GCN/README.md) will be considered, but instead of generating a [NumPy](https://numpy.org/) array, the same image from [TensorFlow image classification tutorial](https://www.tensorflow.org/tutorials/images/classification#predict_on_new_data) prediction step will be used.

The GCN `configuration.json` file remains unaltered, when compared to its [usage example](../GCN/README.md):
...
As for the `camera.py`, it will receive all the necessary code to embrace the [TensorFlow image classification tutorial](https://www.tensorflow.org/tutorials/images/classification#predict_on_new_data) approach:
...
As mentioned, although GCN is not connecting to a camera it assumes its responsibility to *obtain* images, which in this case is the example used in the tutorial and kindly provided by [TensorFlow](https://www.tensorflow.org). Note that the encoding strategy, when compared to [GCN's usage example](../GCN/README.md), remains the same and all the necessary variables to reproduce the tutorial are coded as parameters of the `Camera` class.

As for the `Dockerfile`, [NumPy](https://numpy.org/) is no longer necessary, but [TensorFlow](https://www.tensorflow.org) is:
...
From this point, GCN is ready to be used in the example while ICN still needs to be subject to configuration, starting with its `configuration.json` file:
...
The id `urn:ngsi-ld:ImageRecord:001` needs and does correspond to the [Image Reference Entity](../GCN/data_models/image_reference.json) id configured for GCN.
...
Regarding the `classifier.py` file, it needs to embrace all the necessary code to reproduce the [TensorFlow image classification tutorial](https://www.tensorflow.org/tutorials/images/classification#predict_on_new_data). As already mentioned, ICN only makes use of models and does not train them. For that reason, the `classifier.py` will be prepared to consume models that were saved in the model database service as a `zip` archive containing the files generated by the [TensorFlow's model save function](https://www.tensorflow.org/tutorials/keras/save_and_load#save_the_entire_model) when using the `SavedModel` format, default for `TF2.x.`:
...
The configuration for the remaining `Dockerfile` will include the necessary [TensorFlow](https://www.tensorflow.org) and [NumPy](https://numpy.org/) install commands, since both are used in the `classifier.py` code:
...
Having GCN and ICN Docker images ready, the `docker-compose.yaml` file can be used to create all the service instances:
...
The successful creation of the camera device is proven at [GCN usage example](../GCN/README.md), being here relevant the confirmation of the proper creation of the classifier device entity. To do so, `service` and `service-path` need to be considered when querying Orion:
...
With all the automatic configurations performed by ICN analyzed (GCN was subject to the same analysis in its [usage example](../GCN/README.md)), it is important to check ICN logs by executing the following docker command:
